...
{use GeneralTrait;
    ...
    public function creatFolder(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to create a folder for this project',  );
        }
    ...
    public function reNameFolder(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to reName a folder for this project' );
        }
        if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement &&  $folder->user_id !=$currentUser->id)
        {
            return $this->returnError('You do not have permission to reName a folder for this project Because you didnt create it',  );
        }
        #################  end Test permission ###############################
    ...
    public function deleteFolder(Request $request)
    {
        ...
        try {
            ...
            if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
                return $this->returnError('You do not have permission to delete a folder for this project'  );
            }
            if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement && $folder->user_id !=$currentUser->id)
            {
                return $this->returnError('You do not have permission to delete a folder for this project Because you didnt create it',  );
            }
            #################  end Test permission ###############################
    ...
    public function moveFolder(Request $request)
    {
        ...
        // Retrieve the project
        $project = Project::find($destinationFolder->project_id);
        if (!$project) {
            return $this->returnError('Project not found', 404);
        }
        // Get the currently authenticated user
        ...
        if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement && $destinationFolder->user_id !=$currentUser->id)
        {
            return $this->returnError('You do not have permission to move a folder for this project Because you didnt create it',  );
        }
        #################  end Test permission ###############################
    ...
    public function downloadFolderZip(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to download Folder Zip  for this project');
        }
        #################  end Test permission ###############################
    ...
    public function getMainFolderProjects()
    {
        try {
            ...
            if ($roleNumber == 1) {
                ...
                $projectIds = Project::where('local_coordinator_id',$user->id)->pluck('id')->toArray();
            } elseif ($roleNumber == 2) {
                // إذا كان المستخدم financial
                $projectIds = Project::where('financial_management_id',$user->id)->pluck('id')->toArray();
            } elseif ($roleNumber == 3) {
                // إذا كان المستخدم employ
                $projectIds = $user->projects()->pluck('id')->toArray();
            }

            // الحصول على الحافظات الرئيسية (Main Folders) للمشاريع التي لها معرفات المشاريع المستخدم
            $folders = Folder::where('father_folder_id', null)
    ...
    public function getChildrenFolder(Request $request)
    {
        ...
        if($validate->fails()){
            ...
            $project = Project::find($folder->project_id);
            if (!$project) {
                return $this->returnError('You do not have permission ', 404);
            }
            // Get the currently authenticated user
            $currentUser = Auth::user();
    ...
    public function addFile(Request $request)
    {
        ...
        #################  Test permission ###############################
        // Retrieve the project
        $project = Project::find($father_File->project_id);
        if (!$project) {
            return $this->returnError('Project not found', 404);
        }
        // Get the currently authenticated user
    ...
    public function reNameFile(Request $request)
    {
        ...
        if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement && $File->user_id !=$currentUser->id)
        {
            return $this->returnError('You do not have permission to rename a File for this project Because you didnt create it',  );
        }
        #################  end Test permission ###############################
        // get path father File
        $father_File_path = '';
    ...
    public function deleteFile(Request $request)
    {
        ...
        try {
            ...
            if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
                return $this->returnError('You do not have permission to delete a File for this project');
            }
            if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement && $File->user_id !=$currentUser->id)
            {
                return $this->returnError('You do not have permission to delete a File for this project Because you didnt create it',  );
            }
            #################  end Test permission ###############################
    ...
    public function moveFile(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to move a File for this project');
        }
        if ($isEmployee && !$isLocalCoordinator && !$isFinancialManagement && $file->user_id !=$currentUser->id)
        {
            return $this->returnError('You do not have permission to move a File for this project Because you didnt create it',  );
        }
        #################  end Test permission ###############################
    ...
    public function copyFile(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to copy a File for this project');
        }
        #################  end Test permission ###############################
    ...
    public function downloadFile(Request $request)
    {
        ...
        if (!$isLocalCoordinator && !$isFinancialManagement && !$isEmployee) {
            return $this->returnError('You do not have permission to download a File for this project');
        }
